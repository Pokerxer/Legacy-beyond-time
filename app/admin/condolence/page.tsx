...
import { CheckCircle, XCircle, Trash2, Heart, Plus, Pencil, X, Loader2, Quote } from "lucide-react"
  ...
  const fetchCondolences = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setItems(data.filter((t: Tribute) => t.relationship === "Condolence"))
      }
    } catch {
      // silent
    } finally {
      ...
      >
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            Manage condolence messages submitted from the landing page.
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-4">
          ...
          <p style={{ color: "var(--text-muted)" }}>
            No condolences yet. They will appear here once submitted.
          </p>
        </div>
      ) : (
